...
function connPiper(connection, _dst, opts = {}, stats = {}) {
  ...
  loc.on('data', (d) => {
    logger.log({
      type: 0,
      msg: `Data from local to remote: ${d.length} bytes`
    })
    connection.write(d)
  })
  connection.on('data', (d) => {
    logger.log({
      type: 0,
      msg: `Data from remote to local: ${d.length} bytes`
    })
    loc.write(d)
  })
...
class UdpSocket {
  ...
  write(msg) {
    this.logger.log({
      type: 0,
      msg: `Writing UDP message: ${msg.length} bytes`
    })
    if (this.rinfo) {
...
class UdpConnPiper {
  ...
  onLocMessage(msg, rinfo) {
    this.logger.log({
      type: 0,
      msg: `UDP message from local: ${msg.length} bytes`
    })
  ...
  onConnectionMessage(msg) {
    this.logger.log({
      type: 0,
      msg: `UDP message from connection: ${msg.length} bytes`
    })
  ...
  _handleError(err) {
    this.logger.log({
      type: 2,
      msg: `UDP error: ${err ? err.message : 'close'}`
    })
    this.destroy(err)
  }
  ...
  destroy(err) {
    ...
    if (this.client) {
      this.logger.log({
        type: 1,
        msg: `Scheduling retry in ${this.retryDelay}ms`
      })
      setTimeout(() => {
        this.destroyed = false
        this.connect()
...
function createUdpFramedProxy(listenOpts, connectRemote, logger, stats = {}, onBind) {
  if (!stats.locCnt) stats.locCnt = 0
  if (!stats.remCnt) stats.remCnt = 0
  if (!stats.rejectCnt) stats.rejectCnt = 0
  ...
  proxySocket.on('message', (msg, rinfo) => {
    ...
    logger.log({
      type: 0,
      msg: `UDP message from ${clientId}: ${msg.length} bytes`
    })
    let client = clients.get(clientId)
    if (!client) {
      let remoteStream
      try {
        remoteStream = connectRemote()
      } catch (err) {
        logger.log({
          type: 3,
          msg: `Failed to connect remote for ${clientId}: ${err.stack}`
        })
        stats.rejectCnt++
        return
      }
      if (!remoteStream) {
        logger.log({ type: 2, msg: `Null remote stream for ${clientId}` })
        stats.rejectCnt++
        return
      }
      stats.locCnt++
      stats.remCnt++
      let destroyed = false
      function destroyClient(err) {
        if (destroyed) return
        destroyed = true
        logger.log({
          type: 1,
          msg: `Destroying client ${clientId}${err ? ` due to error: ${err.message}` : ''}`
        })
        stats.locCnt--
        stats.remCnt--
        ...
        while (client.buffer.length >= 4) {
          ...
          proxySocket.send(response, 0, response.length, rinfo.port, rinfo.address, (err) => {
            if (err)
              logger.log({
                type: 3,
                msg: `Send error to ${clientId}: ${err.stack}`
              })
          })
      ...
      remoteStream.on('error', (err) => {
        logger.log({
          type: 3,
          msg: `Remote error for ${clientId}: ${err.stack}`
        })
        destroyClient(err)
      })
...
function pipeUdpFramedServer(remoteStream, localOpts, logger, stats = {}) {
  ...
  function destroy(err) {
    ...
    logger.log({
      type: 1,
      msg: `Destroying UDP server connection${err ? ` due to error: ${err.message}` : ''}`
    })
    stats.locCnt--
    stats.remCnt--
    localSocket.close()
  ...
  localSocket.on('close', () => {
    logger.log({ type: 0, msg: 'Local UDP socket closed' })
    destroy()
  })
  remoteStream.on('error', (err) => {
    logger.log({ type: 3, msg: `Remote error: ${err.stack}` })
    destroy(err)
  })
  remoteStream.on('close', () => {
    logger.log({ type: 0, msg: 'Remote close' })
    destroy()
  })
  ...
  localSocket.on('message', (msg) => {
    logger.log({
      type: 0,
      msg: `Data from local to remote: ${msg.length} bytes`
    })
    const lenBuf = Buffer.alloc(4)
    lenBuf.writeUInt32BE(msg.length, 0)
  ...
  remoteStream.on('data', (chunk) => {
    logger.log({
      type: 0,
      msg: `Data from remote to local: ${chunk.length} bytes`
    })
